...
% allsub=allsub(~ismember(allsub,exclude));
...
for iss1=1:numsession
    ...
    for e = SessionIDX
        ...
        else % plus the mean RT of the trials. 
...
% Time Shift to the recall event.  %% eegoffset + RT
...
orient(h,'landscape')
subplot(1,3,1)
imagesc(uniqueprobe_subID_uniqueword(:,remainsub));
cm=colormap;
cm(1,:)=[1 1 1];
cm(end,:)=[0 0 0];
colormap(cm);
xlabel('Subject');
ylabel('unique Word');title('Stimuli by subjects occurrence');

subplot(1,3,2)
imagesc(uniqueprobe_subID_uniqueword_acc(:,remainsub)>0);
cm=colormap;
cm(1,:)=[1 1 1];
cm(end,:)=[0 0 0];
colormap(cm);
xlabel('Subject');
ylabel('Probe Word');title('Success Recall');

subplot(1,3,3)
imagesc(uniqueresponse_subID_uniqueword_acc(:,remainsub)>0);
cm=colormap;
cm(1,:)=[1 1 1];
cm(end,:)=[0 0 0];
colormap(cm);
xlabel('Subject');
ylabel('Expected Word');title('Success Recall');
