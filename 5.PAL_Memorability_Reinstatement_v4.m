...
curpath=pwd;
outpath=fullfile('/Volumes/Zane/NIH_HPC/NIH_PAL_Mem/NIH_PAL_MEM','data_MTL_3Event_50_5bands');
datapath=outpath;
...
for isubtem = 1:length(ATLIDs)
    ...
    isub
    savefilename=fullfile(datapath,['SubjResults_MTL' num2str(isub) '.mat']);
    
    includedSub(isubtem)=0
    %%
    if exist(savefilename)>0
        
        includedSub(isubtem)=isub;
        ...
        Item1mem = Item1mem(cleanmask>0);
        Item2mem = Item2mem(cleanmask>0);
...
for isubtem = 1:length(ATLIDs)
    ...
    if exist(savefilename)>0
        ...
        correlationRTmemList(isub) = corr((RTnanmeans(nonnanin))',listmemscore(nonnanin)','type','pearson');% more memorable list have retrieved faster.
        correlationACCmemList(isub) = corr(MemlistACC(nonnanin)',listmemscore(nonnanin)','type','pearson'); % more memorable list have higher accuracy.
...
p_late=sum(LateR<0)/niter
p_early=sum(EearlR<0)/niter
p_diff=sum(DiffR<0)/niter
...
p=tpdf(t,stat.df)
        ...
        h=gcf;
        set(h,'PaperOrientation','landscape','Position',[50 50 1200 800]);
        printfilename=fullfile(outpath,['Reinstate_Trace_in_TOI.pdf']);
        print(h,printfilename,'-dpdf','-bestfit');
...
for iter=1:niter
    ...
    PeakAmp1(iter)=max(Y1(tmask));
    PeakAmp2(iter)=max(Y2(tmask));
...
p_peak=1-sum((PeakTime1-PeakTime2)<0)/niter
p_50_fract=1-sum((FractTime1-FractTime2)<0)/niter
p_amp = sum((PeakAmp1-PeakAmp2)<0)/niter
% p_peak=1-sum((PeakTime1-PeakTime2)<0)/niter
% p_50_fract=1-sum((FractTime1-FractTime2)<0)/niter

text(2000,0.12,['pPeak=' num2str(p_peak)])
text(2000,0.11,['pFract=' num2str(p_50_fract)])
text(2000,0.1,['pAmp=' num2str(p_amp)])


h=gcf;
set(h,'PaperOrientation','landscape','Position',[50 50 1200 800]);
printfilename=fullfile(outpath,['Reinstate_Trace_Raw_Stat.pdf']);
print(h,printfilename,'-dpdf','-bestfit');
...
text(50,0.09,['pPeak=' num2str(p_peak)])
text(50,0.08,['pFract=' num2str(p_50_fract)])

h=gcf;
set(h,'PaperOrientation','landscape','Position',[50 50 1200 800]);
printfilename=fullfile(outpath,['Reinstate_Trace_Normalized_Stat.pdf']);
print(h,printfilename,'-dpdf','-bestfit');
